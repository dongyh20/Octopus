...
then
  ...
  LLM="Nous-Hermes-2-Yi-34B-8k"
fi

################## project ##################
PROJECT_NAME="llava-Yi_34B-mlp2x_gelu-pretrain_blip558k_plain-ft-video_image_mix-frames_upbound_${FRAMES_UPBOUND}-pool_stride_${POOL_STRIDE}-model_max_length_${MODEL_MAX_LENGTH}"
...
torchrun --nproc_per_node="${ARNOLD_WORKER_GPU}" --nnodes="${ARNOLD_WORKER_NUM}" --node_rank="${ARNOLD_ID}" --master_addr="${METIS_WORKER_0_HOST}" --master_port="${port_in_cmd}" \
    ...
    --model_name_or_path /mnt/bn/${BYTENAS}/checkpoints/yi/${LLM} \
    ...
    --pretrain_mm_mlp_adapter /mnt/bn/${BYTENAS}/checkpoints/projectors/ds_llava-Nous-Hermes-2-Yi-34B-clip_large_224px-mlp2x_gelu-pretrain_blip558k_plain/mm_projector.bin \
    ...
    --add_image_newline ${ADD_IMAGE_NEWLINE:-True} \
