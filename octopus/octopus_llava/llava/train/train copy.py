...
class EgoGPTDataset(LazySupervisedDataset):
    ...
    def preprocess_imu(self,imu_paths,device,sample_mode='interpolate'):
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        video_feat=egogpt_processor.load_and_transform_video_data(video_path,device)
        audio_feat=egogpt_processor.load_and_transform_aria_audio_data(audio_path,device),
        imu_left_feat=self.preprocess_imu(imu_left_path,device,sample_mode='random'),
        imu_right_feat=self.preprocess_imu(imu_right_path,device,sample_mode='random')

        # image = [(image, video[0].size,"video")]
        sources = self.preprocess_multimodal_egogptdata()

        data_dict = dict(
            video_input=video_feat[0],
            video_label=video_feat[1],
            audio_input=audio_feat[0][0],
            audio_label=audio_feat[0][1],
            imu_left_input=imu_left_feat[0][0],
            imu_left_label=imu_left_feat[0][1],
            imu_right_input=imu_right_feat[0][0],
            imu_right_label=imu_right_feat[0][1]
        ) 
        #TODO add text token
        # if 'prompt' in data_dict:
        #     prompt = data_dict['prompt']
