...
def verify_binary_taskgoal(env,bddl_name1,states,bddl_name2,targets):
    bddl_map = env.task.load_task_metadata()['inst_to_name']
    og_obj1 = bddl_map[bddl_name1] 
    og_obj2 = bddl_map[bddl_name2]
...
def update_log(path, gpt_task_name, finished_type):
    with open(path, "r")as f:
        finished_task = json.load(f)
        finished_task[gpt_task_name] = finished_type
    with open(path, "w")as f:
